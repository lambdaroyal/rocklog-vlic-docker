...
big-persistent:
	...
	-mkdir $(CONT_NAME)/tmp
	-mkdir $(CONT_NAME)/log
	sudo docker run -d -t --restart='always' -p $(COUCHDB_PORT):5984 -p $(VLIC_PORT):8080 -v $(current_dir)/$(CONT_NAME)/data:/data -v $(current_dir)/$(CONT_NAME)/tmp/vlic:/tmp/vlic -v $(current_dir)/$(CONT_NAME)/log:/usr/local/var/log/couchdb/ -v /etc/localtime:/etc/localtime:ro -it vlic/vlic_runner:v1 couchdb big-raw
...
demo:
	...
	-mkdir $(CONT_NAME)/tmp
	-mkdir $(CONT_NAME)/log
	sudo docker run -d -t --restart='always' -p $(COUCHDB_PORT):5984 -p $(VLIC_PORT):8080 -v $(current_dir)/$(CONT_NAME)/data:/data -v $(current_dir)/$(CONT_NAME)/tmp/vlic:/tmp/vlic -v $(current_dir)/$(CONT_NAME)/log:/usr/local/var/log/couchdb/ -v /etc/localtime:/etc/localtime:ro -it vlic/vlic_runner:v1 couchdb demo
